...
class DataIngestionConfig:
   ...
   raw_data_path:str=os.path.join('artfacts','raw_data.csv')
...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            ...
            os.makedirs(os.path.dirname(self.ingestion_config.train_data_path),exist_ok=True) #this will make the directory for train data
            
            df.to_csv(self.ingestion_config.raw_data_path,index=False,header=True)
            
            logging.info("Train test has been initiated")
            train_set,test_set=train_test_split(df,test_size=0.2,random_state=42)
            
            train_set.to_csv(self.ingestion_config.train_data_path,index=False,header=True)
            test_set.to_csv(self.ingestion_config.test_data_path,index=False,header=True)
            
            logging.info("Ingestions of the data is completed")
